	...
	try:
		...
		if frappe.local.flags.in_migrate:
			...
			def _run_in_memory():
				...
				print(f"[InMemoryWorker] Queuing job {m_name} (event: {event}) to ThreadPool...")

				future = _in_memory_pool.submit(
					execute_job,
				...
				def _log_if_failed(f):
					try:
						f.result()
						print(f"[InMemoryWorker] Job {m_name} completed successfully.")
					except Exception as e:
						import traceback
						print(f"[InMemoryWorker] Job {m_name} FAILED with exception: {e}")
						traceback.print_exc()

				future.add_done_callback(_log_if_failed)

			if enqueue_after_commit:
				print(f"[InMemoryWorker] Deferring job to after_commit hook...")
